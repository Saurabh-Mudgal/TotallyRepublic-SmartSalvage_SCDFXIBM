...
import { Car } from "../../client/classes/Car.js";
import { Graph } from "../../client/classes/Graph.js";
import { Node } from "../../client/classes/Node.js";
import { Road } from "../../client/classes/Road.js";
import { Traffic } from "../../client/algorithms/routingAlgorithm.js";
...
function main() {
    ...
    for (var i = 0; i < 5; i++) {
        ...
        for (var j = 0; j < 5; j++) {
            ...
            var node = new Node({ x: -130 + 90 * i, y: -130 + 90 * j }, type);
            row.push(node);
            nodes.push(node);
        }
        basicGraph.push(row);
    ...
    console.log(nodes);
    var roads = [];
    // console.log(basicGraph[0][1], basicGraph[0][0]);
    // var testRoad = new Road(basicGraph[0][1], basicGraph[0][0], 1, 10);
    // console.log(testRoad);
    for (var i = 0; i < 5; i++) {
        for (j = 0; j < 4; j++) {
            var road = new Road(basicGraph[i][j + 1], basicGraph[i][j], 1, 10);
            roads.push(road);
        }
    }
    ...
    for (var j = 0; j < 5; j++) {
        for (var i = 0; i < 4; i++) {
            var road = new Road(basicGraph[i + 1][j], basicGraph[i][j], 1, 10);
            roads.push(road);
        }
    }
    console.log(roads);

    var myRoadGraph = new Graph(nodes, roads, false);
    var myTraffic = new Traffic(myRoadGraph, 1, 10);
    ...
    // setInterval(nextIteration, 5000);
    // console.log(myTraffic.render());
    // nextIteration();
    // nextIteration();
    // nextIteration();
    // nextIteration();
    ...
    function nextIteration() {
        var cars = myTraffic.render();
        console.log(cars);
        var carKeys = Object.keys(cars);
        for (var i = 0; i < carKeys.length; i++) {
            var currKey = carKeys[i];
            var currCar = cars[currKey];
            if (!carObjects[currKey]) {
                const gltfLoader = new GLTFLoader();
                gltfLoader.load("./assets/anime_model/scene.gltf", (gltf) => {
                    var currRoot = gltf.scene;
                    currRoot.scale.set(0.5, 0.5, 0.5);
                    scene.add(currRoot);
                    currRoot.position.set(
                        currCar.position.x,
                        0,
                        currCar.position.y
                    );
                    carObjects[currKey] = currRoot;
                });
            } else {
                translator(carObjects[currKey], {
                    x: currCar.position.x,
                    y: 0,
                    z: currCar.position.y,
                });
            }
        }
    }
    ...
    let doll_root;
    {
        ...
        gltfLoader2.load("./assets/anime_model/scene.gltf", (gltf) => {
            ...
            doll_root.position.set(-40, 0, -130);
        });
    }
    // let bike;
    // {
